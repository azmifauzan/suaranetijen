...
class MediaKonsumenAdapter extends AbstractHttpSourceAdapter
{
    protected function preflightUrl(): string
    {
        return rtrim((string) config('sources.mediakonsumen.base_url'), '/').'/';
    }

    public function discover(CrawlCursor $cursor): DiscoveryBatch
    {
        $page = max(1, (int) ($cursor->metadata['page'] ?? 1));
        $feedUrl = (string) ($cursor->metadata['feed_url'] ?? config('sources.mediakonsumen.feed_url'));
        // WordPress feed pagination uses ?paged=N, not the ?page=N that
        // AbstractHttpSourceAdapter::pageUrl() builds for forum-style
        // pagination — page= is silently ignored on a feed endpoint.
        $url = $page > 1
